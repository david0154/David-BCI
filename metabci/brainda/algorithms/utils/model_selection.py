...
def generate_kfold_indices(
    ...
    """The EnhancedStratifiedKFold class is invoked at the david data structure level
    to generate cross-validation grouping subscripts.
    The subscript of K-fold cross-validation is generated based on david class data structure.
    ...
    Parameters
    ----------
    david: pandas.DataFrame
        davidbci's custom data class.
    kfold: int
        Cross validation fold, default is 5.
    ...
    """
    subjects = david["subject"].unique()
    event_names = david["event"].unique()
    indices = {}

    for sub_id in subjects:
        sub_ix = david["subject"] == sub_id
        classes_indices = {}
        for e_name in event_names:
            k_indices = []
            ix = sub_ix & (david["event"] == e_name)
...
def match_kfold_indices(k: int, david: DataFrame, indices):
    """At the level of david data structure,
    hierarchical K-fold cross-validation packet subscripts are matched to generate specific indexes.
    Based on david class data structure and combined with the output results of generate_kfold_indices(),
    ...
    train_ix: ndarray, ‘subject id’: classes_indices
        ...
        of the full class data of all subjects (i.e., david-class data).
    val_ix: ndarray, ‘subject id’: classes_indices
        The validation set trial index required for validation of the david-class data at k-fold validation.
    test_ix: ndarray, ‘subject id’: classes_indices
        The test set trial index required for validation of the david-class data at the k-fold.
    """
    ...
    for sub_id in subjects:
        for e_name in event_names:
            sub_meta = david[(david["subject"] == sub_id) & (david["event"] == e_name)]
            train_ix.append(
...
def generate_loo_indices(david: DataFrame):
    """
    The EnhancedLeaveOneGroupOut class is invoked at the david data structure level
    to generate cross-validation grouping subscripts.
    The subscript of leave-one method cross-validation is generated based on david class data structure.
...
def match_loo_indices(k: int, david: DataFrame, indices):
    """
    At the david data structure level, a method is matched
    to cross-validate the grouping subscript and generate the specific index.
    Based on the david class data structure and combined with the output of generate_loo_indices(),
    the specific index is generated.
    ...
    -------
    train_ix: ndarray, ‘subject id’: classes_indices
        The index of the training set trial required by the k-fold verification of david class data.
    val_ix: ndarray, ‘subject id’: classes_indices
        The validation set trial index required for validation of the david-class data at k-fold validation.
    test_ix: ndarray, ‘subject id’: classes_indices
        The test set trial index required for validation of the david-class data at the k-fold.
...
def generate_shuffle_indices(
    ...
    """
    Level in the david data structure called EnhancedStratifiedShuffleSplit class,
    generating cross validation grouping subscript.
    Generate hierarchical random cross-validation subscripts based on david-class data structures.
...
def match_shuffle_indices(k: int, david: DataFrame, indices):
    """
    Random cross-validation grouping subscripts are matched at the david data structure level
    to generate specific indexes.
    Based on the david class data structure and combined with the output of generate_shuffle_indices(),
...
def generate_char_indices(
    ...
    for sub_id in subjects:
        sub_ix = david["subject"] == sub_id
        # classes_indices = {}
        # char_total = david.event.__len__()
        k_indices = []
        ix = sub_ix
        spliter = EnhancedStratifiedKFold(
            n_splits=kfold, shuffle=True, random_state=random_state
...
def match_char_kfold_indices(k: int, david: DataFrame, indices):
    ...
    subjects = david["subject"].unique()
    for sub_id in subjects:
        sub_meta = david[(david["subject"] == sub_id)]
        train_ix.append(
