...
the similarity of the aliasing matrix across frequencies to enhance the performance of SSVEP-DBCI with
...
def get_augment_noiseAfter(fs, f, Nh, n_Aug, mean_temp, alpha=0.05):
    ...
    Note
    ----
    Please note that we apply SAME before filter bank analysis in the DavidBCI version.
    This is convenient for compatibility with DavidBCI and saves computational effort.
...
class SAME(BaseEstimator, TransformerMixin):
    ...
    .. code-block:: python
       ...
       :caption: A example using SAME
        ...
        from davidbci.brainda.algorithms.transfer_learning import SAME
...
def get_augment_noiseAfter_ms(fs, f_list, phi_list, Nh, n_Aug, mean_temp_all, iEvent, n_Templates, alpha=0.05):
    ...
    Note
    ----
    Please note that we apply msSAME before filter bank analysis in the DavidBCI version.
    This is convenient for compatibility with DavidBCI and saves computational effort.
    After testing, it still has a similar improvement effect.
...
class MSSAME(BaseEstimator, TransformerMixin):
    ...
    .. code-block:: python
       ...
       :caption: A example using MSSAME
        ...
        from davidbci.brainda.algorithms.transfer_learning import MSSAME
